...
/* ── Delete Announcement ── */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'delete_announcement') {
    while (ob_get_level()) ob_end_clean();
    header('Content-Type: application/json');
    if (empty($_SESSION['admin_logged_in'])) { echo json_encode(['success'=>false,'message'=>'Unauthorized.']); exit; }
    $announcementId = (int)($_POST['announcement_id'] ?? 0);
    if ($announcementId <= 0) { echo json_encode(['success'=>false,'message'=>'Invalid announcement.']); exit; }
    $stmt = $pdo->prepare("DELETE FROM announcements WHERE id = ?");
    $stmt->execute([$announcementId]);
    if ($stmt->rowCount() < 1) { echo json_encode(['success'=>false,'message'=>'Announcement not found.']); exit; }
    logActivity($pdo, 'ANNOUNCEMENT_DELETED', "Announcement #$announcementId was deleted.", 'announcement', $announcementId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'upload_file') {
    ...
    $upload_dir  = $appRoot . '/uploads/files/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'get_files') {
    ...
    try {
        ...
        foreach ($files as &$f) {
            ...
            $f['exists'] = file_exists($appRoot . '/uploads/files/' . $f['stored_name']);
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'upload_software_pdf') {
    ...
    header('Content-Type: application/json');
    if (empty($_SESSION['admin_logged_in'])) { echo json_encode(['success'=>false,'message'=>'Unauthorized.']); exit; }
    $upload_dir = $appRoot . '/uploads/software/';
    if (!is_dir($upload_dir)) mkdir($upload_dir, 0755, true);
    ...
    try {
        ...
        if ($oldPdf && $oldPdf !== $filename && file_exists($appRoot . '/uploads/software/' . $oldPdf)) @unlink($appRoot . '/uploads/software/' . $oldPdf);
...
if (isset($_GET['export'])) {
    ...
    function outputPDF($data, $headers, $title, $filename) {
        try {
            if (class_exists('TCPDF')) {
                ...
                $pdf->SetTitle($title);
                $pdf->SetHeaderData('', 0, $title, 'Generated on ' . date('F d, Y h:i A'));
                ...
                $html = '<h2 style="font-size:16px;">' . htmlspecialchars($title) . '</h2>';
                $html .= '<table border="1" cellpadding="4" style="border-collapse: collapse; width: 100%;"><thead><tr style="background-color:#f1f0f7;">';
                foreach ($headers as $header) {
    ...
    if ($export_type === 'analytics') {
        ...
        $headers = ['Report Type', 'Date', 'Count', 'Notes', 'Laboratory', 'Total Sit-ins', 'Unique Students', 'Course', 'Metric', 'Value'];
...
if ($sessionRole !== 'admin' || empty($_SESSION['admin_logged_in'])) {
...
.dash-body{display:flex;min-height:100vh;background:radial-gradient(circle at top left,rgba(20,184,166,.12),transparent 24%),radial-gradient(circle at bottom right,rgba(245,158,11,.10),transparent 18%),linear-gradient(180deg,#f7fdfc 0%,#edf8f7 100%);overflow-x:hidden}
...
/* ── FILE UPLOADS PAGE ── */
.file-upload-drop{border:2.5px dashed #99f6e4;border-radius:14px;padding:2rem 1.5rem;text-align:center;cursor:pointer;transition:border-color .2s,background .2s;background:#f0fdfa}
.file-upload-drop:hover,.file-upload-drop.drag-over{border-color:var(--purple-mid);background:#ecfeff}
.file-upload-drop i{font-size:2.5rem;color:#2dd4bf;display:block;margin-bottom:.75rem}
...
.ftb-other{background:#ccfbf1;color:var(--purple-mid)}
.file-card{background:#fff;border-radius:12px;padding:1rem 1.2rem;border:1px solid #dcecea;display:flex;align-items:center;gap:1rem;transition:box-shadow .18s}
.file-card:hover{box-shadow:0 4px 20px rgba(15,118,110,.1)}
...
.upload-progress{height:4px;background:#ccfbf1;border-radius:99px;overflow:hidden;margin-top:.4rem;display:none}
...
/* ── SIDEBAR LABEL ── */
.sidebar{width:252px;min-width:252px;background:rgba(255,255,255,.9);backdrop-filter:blur(18px);border-right:1px solid #dcecea;display:flex;flex-direction:column;height:100vh;position:sticky;top:0;z-index:300;box-shadow:12px 0 40px rgba(15,118,110,.08);transition:transform .28s cubic-bezier(.4,0,.2,1)}
.sb-brand{display:flex;align-items:center;gap:.8rem;padding:1.35rem 1.2rem 1rem;border-bottom:1px solid #dcecea}
...
.sb-nav{padding:.9rem .75rem 0;flex:1;overflow-y:auto}
.sb-nav ul{list-style:none;display:flex;flex-direction:column;gap:.15rem}
.sb-link{display:flex;align-items:center;gap:.8rem;padding:.7rem .95rem;border-radius:14px;text-decoration:none;color:#4b5563;font-size:.875rem;font-weight:600;transition:all .2s;position:relative;cursor:pointer}
.sb-link:hover{background:#eefaf8;color:var(--purple-mid);transform:translateX(2px)}
.sb-link.active{background:linear-gradient(135deg,#e6fffb,#ffffff);color:var(--purple-mid);font-weight:700;box-shadow:0 10px 24px rgba(15,118,110,.1)}
...
.page-title{font-family:var(--ff);font-size:1.72rem;font-weight:800;color:var(--text);letter-spacing:-.03em;line-height:1.08}
.page-sub{color:var(--muted);font-size:.9rem;margin-top:.28rem;line-height:1.55;max-width:660px}
.page-actions{display:flex;align-items:center;gap:.75rem;flex-wrap:wrap}
.welcome-banner{background:radial-gradient(circle at top right,rgba(255,255,255,.14),transparent 26%),linear-gradient(135deg,#09282c 0%,var(--purple-dark) 45%,var(--purple-mid) 100%);border-radius:26px;padding:2rem 2rem 1.7rem;display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:1.2rem;position:relative;overflow:hidden;box-shadow:0 24px 60px rgba(15,118,110,.26);border:1px solid rgba(255,255,255,.10)}
.welcome-banner::after{content:'';position:absolute;right:-40px;top:-40px;width:220px;height:220px;border-radius:50%;background:rgba(255,255,255,.05)}
.welcome-title{font-family:var(--ff);font-size:clamp(1.9rem,3vw,2.35rem);font-weight:800;color:#fff;letter-spacing:-.04em;line-height:1.02}
.welcome-sub{font-size:.92rem;color:rgba(255,255,255,.76);margin-top:.55rem;line-height:1.6}
.badge-admin{background:linear-gradient(135deg,#fef3c7,#fdba74);color:#92400e;font-family:var(--ff);font-size:.8rem;font-weight:800;padding:.42rem .9rem;border-radius:999px;letter-spacing:.05em;box-shadow:0 10px 22px rgba(245,158,11,.18)}
.home-grid{display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;margin-bottom:1.5rem}
.dash-card{background:rgba(255,255,255,.94);border-radius:22px;padding:1.3rem 1.5rem;box-shadow:0 14px 34px rgba(15,118,110,.06);border:1px solid #dcecea;backdrop-filter:blur(14px)}
...
.stat-card{background:linear-gradient(145deg,#fff,#f3fcfa);border-radius:20px;padding:1.15rem 1.3rem;display:flex;align-items:center;gap:.95rem;border:1px solid #dcecea;box-shadow:0 12px 28px rgba(0,0,0,.05);transition:transform .2s,box-shadow .2s}
.stat-card:hover{transform:translateY(-4px);box-shadow:0 18px 38px rgba(15,118,110,.10)}
...
.sc-icon{width:44px;height:44px;border-radius:13px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0}
.stat-card.p .sc-icon{background:#ccfbf1;color:var(--purple-mid)}
...
.ann-textarea:focus{border-color:var(--purple-mid);box-shadow:0 0 0 3px rgba(15,118,110,.1)}
.ann-list{display:flex;flex-direction:column;gap:.6rem;max-height:280px;overflow-y:auto}
.ann-item{padding:.75rem 1rem;border-radius:10px;background:#fafdfd;border:1px solid #e5f3f1}
...
.data-table thead tr{background:#f3fcfa;border-bottom:2px solid #dcecea}
...
.btn-outline-sm:hover{border-color:var(--purple-mid);color:var(--purple-mid);background:#eefaf8}
...
.search-input{padding:.6rem .95rem .6rem 2.2rem;border:1.5px solid var(--border);border-radius:12px;font-family:var(--fb);font-size:.84rem;color:var(--text);background:#fafafa;outline:none;width:220px;transition:border-color .2s,box-shadow .2s}
.search-input:focus{border-color:var(--purple-mid);box-shadow:0 0 0 3px rgba(15,118,110,.1);background:var(--white);width:260px}
.table-toolbar{display:flex;align-items:center;justify-content:space-between;gap:.8rem;flex-wrap:wrap;padding:1rem 1.5rem .85rem;background:linear-gradient(180deg,#fcfbff 0%,#f8f5ff 100%);border-bottom:1px solid #f0ecff}
...
.filter-select{padding:.52rem .78rem;border:1.5px solid var(--border);border-radius:12px;font-size:.8rem;font-family:var(--fb);background:white;cursor:pointer;box-shadow:0 6px 16px rgba(39,24,86,.04);transition:border-color .16s,box-shadow .16s}
.filter-select:focus{outline:none;border-color:var(--purple-mid);box-shadow:0 0 0 4px rgba(108,63,207,.10)}
.stats-detail-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.5rem}
.stats-detail-card{background:linear-gradient(145deg,#ffffff,#faf8ff);border-radius:20px;padding:1.05rem 1rem;text-align:left;border:1px solid #e9e3fb;box-shadow:0 12px 28px rgba(42,24,96,.05);position:relative;overflow:hidden}
.stats-detail-card::after{content:'';position:absolute;right:-24px;bottom:-30px;width:92px;height:92px;border-radius:50%;background:rgba(108,63,207,.05)}
.stats-detail-value{font-family:var(--ff);font-size:1.22rem;font-weight:800;color:var(--purple-mid);position:relative;z-index:1}
.stats-detail-label{font-size:.72rem;color:var(--muted);margin-top:.28rem;position:relative;z-index:1}
.stats-detail-card.accent-green .stats-detail-value{color:#15803d}
.stats-detail-card.accent-yellow .stats-detail-value{color:#a16207}
.stats-detail-card.accent-red .stats-detail-value{color:#dc2626}
...
.rsv-tab-btn.active{background:var(--white);color:var(--purple-mid);box-shadow:0 10px 18px rgba(108,63,207,.12);font-weight:700}
...
<main class="dash-main">
  ...
  <div class="dash-page active" id="page-home">
    <div class="welcome-banner">
      <div style="position:relative;z-index:1;max-width:640px">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.8rem;margin-top:1.1rem;max-width:560px">
          <div style="padding:.9rem 1rem;border-radius:16px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.14)">
            <div style="font-size:.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.7)">Students</div>
            <div style="font-family:var(--ff);font-size:1.3rem;font-weight:800;color:#fff;margin-top:.35rem"><?= $total ?></div>
          </div>
          <div style="padding:.9rem 1rem;border-radius:16px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.14)">
            <div style="font-size:.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.7)">Active Now</div>
            <div style="font-family:var(--ff);font-size:1.3rem;font-weight:800;color:#fff;margin-top:.35rem"><?= $currently ?></div>
          </div>
          <div style="padding:.9rem 1rem;border-radius:16px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.14)">
            <div style="font-size:.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.7)">Total Logs</div>
            <div style="font-family:var(--ff);font-size:1.3rem;font-weight:800;color:#fff;margin-top:.35rem"><?= $total_sitin ?></div>
          </div>
        </div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.55rem;position:relative;z-index:1">

        <div style="padding:.55rem .85rem;border-radius:999px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.12);font-size:.78rem;font-weight:700;color:#fff">Live Operations Enabled</div>
      </div>
    </div>
    <div class="stats-row">
    ...
    <div class="home-grid">
      ...
      <div class="dash-card">
        ...
        <div class="ann-list" id="annList">
          ...
          <div class="ann-item" data-announcement-id="<?= (int)$a['id'] ?>">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.75rem">
              <div class="ann-meta"><?= htmlspecialchars($a['admin_name']) ?> | <?= date('M d, Y', strtotime($a['created_at'])) ?></div>
              <button class="btn-danger-sm delete-ann-btn" data-announcement-id="<?= (int)$a['id'] ?>" style="padding:.26rem .65rem;font-size:.72rem;flex-shrink:0"><i class="fa-solid fa-trash"></i> Delete</button>
            </div>
  ...
  <div class="dash-page" id="page-reports">
    ...
    <div class="stats-detail-grid">
      ...
      <div class="stats-detail-card accent-green"><div class="stats-detail-value" id="totalSitinsAnalytics"><?= $total_sitin ?></div><div class="stats-detail-label">Total Sit-ins</div></div>
      <div class="stats-detail-card accent-yellow"><div class="stats-detail-value"><?= $total > 0 ? round($total_sitin / $total, 1) : 0 ?></div><div class="stats-detail-label">Avg Sit-ins/Student</div></div>
      <div class="stats-detail-card accent-red"><div class="stats-detail-value"><?= $currently ?></div><div class="stats-detail-label">Currently Active</div></div>
    </div>
  ...
  <div class="stats-detail-card accent-green">
  ...
  <div class="stats-detail-card accent-yellow">
  ...
  <div class="stats-detail-card accent-red">
    ...
    <div class="home-grid" style="margin-bottom:1.5rem">
      ...
      <div class="dash-card"><div class="card-header"><h2><i class="fa-solid fa-chart-simple"></i> Hourly Activity Trend</h2></div><canvas id="hourlyChart" height="180"></canvas></div>
    </div>
    
    <div class="home-grid" style="margin-bottom:1.5rem">
      <div class="dash-card"><div class="card-header"><h2><i class="fa-solid fa-graduation-cap"></i> Course Distribution</h2></div><canvas id="courseDistChart" height="108"></canvas></div>
      <div class="dash-card"><div class="card-header"><h2><i class="fa-solid fa-chart-pie"></i> Year Level Distribution</h2></div><canvas id="yearDistChart" height="108"></canvas></div>
    </div>
  ...
  <div class="dash-page" id="page-reservation">
    ...
    <div class="rsv-tab-panel" id="rsv-tab-requests" style="display:block">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;margin:1rem 0 1rem;padding:1rem 1.15rem;border-radius:18px;background:linear-gradient(135deg,#ffffff,#f7f3ff);border:1px solid #eee7ff">
      ...
      <div class="reservation-cards-grid" id="reservationCardsGrid">
        ...
        <div class="reservation-card" data-status="<?= $r['status'] ?>" data-lab="<?= htmlspecialchars($r['lab']??'') ?>" data-student-id="<?= $r['student_db_id'] ?>" data-reservation-id="<?= $r['id'] ?>">
          <div style="background:linear-gradient(135deg,#5630b4,#6c3fcf,#a259f7);padding:1rem 1.2rem;color:#fff">
    ...
    <div class="rsv-tab-panel" id="rsv-tab-logs" style="display:none">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;margin:1rem 0 .75rem;padding:1rem 1.15rem;border-radius:18px;background:linear-gradient(135deg,#ffffff,#f7f3ff);border:1px solid #eee7ff">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ANNOUNCEMENT */
  function formatAnnouncementDate(dateInput) {
  ...
  function renderAnnouncementItem(announcement) {
    const adminName = escapeHtml(announcement.admin_name || 'Admin');
    const content = escapeHtml(announcement.content || '');
    const dateLabel = formatAnnouncementDate(announcement.created_at);
    return `<div class="ann-item" data-announcement-id="${Number(announcement.id || 0)}">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.75rem">
        <div class="ann-meta">${adminName} | ${dateLabel}</div>
        <button class="btn-danger-sm delete-ann-btn" data-announcement-id="${Number(announcement.id || 0)}" style="padding:.26rem .65rem;font-size:.72rem;flex-shrink:0"><i class="fa-solid fa-trash"></i> Delete</button>
      </div>
      <div class="ann-text">${content}</div>
    </div>`;
  }
  ...
  document.getElementById('annSubmitBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        item.innerHTML = renderAnnouncementItem(data.announcement || { id:0, admin_name:'Admin', content:text, created_at:new Date().toISOString() });
        list.prepend(item.firstElementChild);
      } else showToast(data.message,'error');
    } catch { showToast('Server error.','error'); }
  });

  document.getElementById('annList')?.addEventListener('click', async (e) => {
    const btn = e.target.closest('.delete-ann-btn');
    if (!btn) return;
    const announcementId = Number(btn.dataset.announcementId || 0);
    if (!announcementId) { showToast('Invalid announcement.', 'error'); return; }
    ...
    fd.append('_action', 'delete_announcement');
    ...
    try {
      ...
      item?.remove();
      const list = document.getElementById('annList');
      if (list && !list.querySelector('.ann-item')) {
  ...
  function loadAnalyticsCharts() {
    ...
    if (courseDistData.length && document.getElementById('courseDistChart')) {
      ...
      courseDistChart = new Chart(document.getElementById('courseDistChart'), {
        type:'pie',
        data:{
          labels,
          datasets:[{
        ...
        options:{
          responsive:true,
          maintainAspectRatio:true,
          plugins:{ legend:{ position:'right', labels:{ font:{ size:9 }, boxWidth:8, boxHeight:8, padding:8 } } }
        }
      });
    }
    if (yearDistData.length && document.getElementById('yearDistChart')) {
      ...
      yearDistChart = new Chart(document.getElementById('yearDistChart'), {
        type:'doughnut',
        data:{
          labels,
          datasets:[{
