...
$upload_dir = $appRoot . '/uploads/profiles/';
...
if ($sessionRole !== 'student' || empty($_SESSION['student_id'])) {
    ob_clean();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'update_profile') {
    ...
    if ($remove_photo) {
        ...
        if (!empty($old_row['profile_photo'])) { $old_file = $appRoot . '/' . $old_row['profile_photo']; if (file_exists($old_file)) @unlink($old_file); }
        ...
        $dest_dir = $appRoot . '/uploads/profiles/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'get_software_pdf') {
    ...
    try {
        ...
        if ($pdf && file_exists($appRoot . '/uploads/software/' . $pdf)) {
            ob_clean();
...
if (isset($_GET['view_file'])) {
    ...
    try {
        ...
        $path = $appRoot . '/uploads/files/' . $f['stored_name'];
...
$all_labs = [];
try {
    $all_labs = $pdo->query("SELECT id, name, capacity, is_active, software_list FROM labs ORDER BY name ASC")->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $_e) {}

$lab_software_map = [];
foreach ($all_labs as $lab_row) {
    $decoded = json_decode($lab_row['software_list'] ?? '[]', true);
    $lab_software_map[$lab_row['name']] = is_array($decoded) ? array_values(array_filter($decoded, fn($item) => is_string($item) && trim($item) !== '')) : [];
}
...
<head>
  ...
  <style>
:root{--purple-dark:#0b2f33;--purple-mid:#0f766e;--purple-light:#14b8a6;--yellow:#f59e0b;--red:#ef4444;--green:#22c55e;--text:#1a1a2e;--muted:#6b7280;--border:#dbe7e5;--white:#ffffff;--ff:Arial,Helvetica,sans-serif;--fb:Arial,Helvetica,sans-serif}
/* ── LAYOUT ── */
.dash-body{display:flex;min-height:100vh;background:
radial-gradient(circle at top left,rgba(20,184,166,.12),transparent 24%),
radial-gradient(circle at bottom right,rgba(245,158,11,.10),transparent 18%),
linear-gradient(180deg,#f7fdfc 0%,#edf8f7 100%);overflow-x:hidden}
.sidebar{width:252px;min-width:252px;background:rgba(255,255,255,.9);backdrop-filter:blur(18px);border-right:1px solid rgba(236,233,248,.9);display:flex;flex-direction:column;height:100vh;position:sticky;top:0;z-index:300;box-shadow:10px 0 40px rgba(76,46,143,.08);transition:transform .28s cubic-bezier(.4,0,.2,1)}
.sb-brand{display:flex;align-items:center;gap:.85rem;padding:1.35rem 1.2rem 1rem;border-bottom:1px solid #ece9f8}
...
.sb-sub{font-size:.68rem;color:var(--muted);font-weight:500;letter-spacing:.03em;text-transform:uppercase}
.sb-nav{padding:.9rem .75rem 0;flex:1}
.sb-nav ul{list-style:none;display:flex;flex-direction:column;gap:.15rem}
.sb-link{display:flex;align-items:center;gap:.8rem;padding:.68rem .95rem;border-radius:14px;text-decoration:none;color:#4b5563;font-size:.875rem;font-weight:600;transition:all .2s;position:relative}
.sb-link:hover{background:#eefaf8;color:var(--purple-mid);transform:translateX(2px)}
.sb-link.active{background:linear-gradient(135deg,#e6fffb,#ffffff);color:var(--purple-mid);font-weight:700;box-shadow:0 10px 22px rgba(15,118,110,.10)}
...
.sb-icon{width:32px;height:32px;display:flex;align-items:center;justify-content:center;border-radius:10px;font-size:.9rem;flex-shrink:0;background:rgba(15,118,110,.05);transition:background .2s,transform .2s}
.sb-link.active .sb-icon{background:rgba(15,118,110,.12)}
.sb-link:hover .sb-icon{background:rgba(15,118,110,.09);transform:scale(1.04)}
.sb-spacer{flex:1}
...
.sb-user-btn:hover{background:#edf9f7}
.sb-avatar{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--purple-mid),var(--purple-light));color:#fff;font-family:var(--ff);font-size:.95rem;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(15,118,110,.32)}
...
.dash-main{flex:1;min-width:0;padding:1.8rem 2rem 2.4rem;overflow-y:auto}
.dash-page{display:none}
.dash-page.active{display:block;animation:fadeUp .3s ease both}
@keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
.page-header{display:flex;align-items:flex-start;justify-content:space-between;margin-top:1rem;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;padding-bottom:.35rem}
.page-title{font-family:var(--ff);font-size:1.7rem;font-weight:800;color:var(--text);letter-spacing:-.03em;line-height:1.1}
.page-sub{color:var(--muted);font-size:.9rem;margin-top:.28rem;max-width:640px;line-height:1.55}
...
linear-gradient(135deg,#0b2f33 0%,#0f766e 52%,#14b8a6 100%);border-radius:26px;padding:2rem 2rem 1.6rem;display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1.4rem;flex-wrap:wrap;gap:1.2rem;position:relative;overflow:hidden;box-shadow:0 22px 60px rgba(15,118,110,.28);border:1px solid rgba(255,255,255,.12)}
.welcome-banner::before{content:'';position:absolute;inset:auto auto -48px -30px;width:180px;height:180px;background:rgba(245,158,11,.12);filter:blur(8px);border-radius:50%;pointer-events:none}
.welcome-banner::after{content:'';position:absolute;right:-40px;top:-40px;width:220px;height:220px;background:rgba(255,255,255,.05);border-radius:50%;pointer-events:none}
.welcome-text{position:relative;z-index:1;max-width:620px}
.welcome-eyebrow{font-size:.78rem;color:rgba(255,255,255,.72);font-weight:700;margin-bottom:.45rem;letter-spacing:.12em;text-transform:uppercase}
.welcome-title{font-family:var(--ff);font-size:clamp(1.8rem,3vw,2.35rem);font-weight:800;color:#fff;letter-spacing:-.04em;line-height:1.02}
.welcome-title span{color:#f59e0b}
.welcome-sub{font-size:.93rem;color:rgba(255,255,255,.8);margin-top:.6rem;line-height:1.6}
.welcome-actions{display:flex;flex-wrap:wrap;gap:.7rem;margin-top:1rem}
.hero-mini-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.8rem;margin-top:1.15rem;width:min(100%,540px)}
.mini-stat{padding:.9rem 1rem;border-radius:16px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(10px)}
.mini-stat-label{display:flex;align-items:center;gap:.4rem;font-size:.72rem;font-weight:700;color:rgba(255,255,255,.7);text-transform:uppercase;letter-spacing:.08em}
.mini-stat-value{display:block;font-family:var(--ff);font-size:1.3rem;font-weight:800;color:#fff;margin-top:.45rem;letter-spacing:-.03em}
.welcome-badge{display:flex;flex-direction:column;align-items:flex-end;gap:.55rem;z-index:1;position:relative}
.badge-course{background:#f59e0b;color:#1a1a00;font-family:var(--ff);font-size:.82rem;font-weight:800;padding:.42rem .9rem;border-radius:999px;letter-spacing:.05em;box-shadow:0 8px 20px rgba(245,158,11,.22)}
.badge-id{font-size:.8rem;color:rgba(255,255,255,.72);font-family:monospace;letter-spacing:.14em}
.welcome-status{display:flex;align-items:center;gap:.45rem;padding:.42rem .75rem;border-radius:999px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.12);color:#fff;font-size:.78rem;font-weight:700}
.welcome-status i{color:#86efac}

/* Stat cards */
.stat-cards{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.5rem}
.stat-card{border-radius:20px;padding:1.25rem 1.3rem;display:flex;align-items:center;gap:1rem;position:relative;overflow:hidden;transition:transform .22s,box-shadow .22s;border:1px solid transparent;box-shadow:0 14px 30px rgba(46,24,90,.06)}
.stat-card:hover{transform:translateY(-4px);box-shadow:0 22px 42px rgba(46,24,90,.10)}
.stat-card.purple{background:linear-gradient(145deg,#f0fdfa,#ccfbf1);border-color:#99f6e4}
.stat-card.yellow{background:linear-gradient(145deg,#fffdf1,#fff1bf);border-color:#fde68a}
.stat-card.green{background:linear-gradient(145deg,#f4fff7,#dcfce7);border-color:#b7f0c8}
.stat-card.pink{background:linear-gradient(145deg,#fff6fb,#fce7f3);border-color:#f8b8d8}
...
.stat-card.purple .sc-icon{background:rgba(15,118,110,.15);color:var(--purple-mid)}
...
.home-grid{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(360px,.85fr);gap:1.25rem;align-items:start}

/* Cards */
.dash-card{background:rgba(255,255,255,.94);backdrop-filter:blur(14px);border-radius:22px;padding:1.35rem 1.5rem;box-shadow:0 12px 34px rgba(34,24,72,.06);border:1px solid rgba(236,233,248,.95);transition:transform .22s,box-shadow .22s}
.dash-card:hover{transform:translateY(-2px);box-shadow:0 18px 42px rgba(34,24,72,.10)}
.dash-card.no-pad{padding:0}
.dash-card.no-pad .card-header{padding:1.1rem 1.5rem}
.card-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem}
.card-header h2{font-family:var(--ff);font-size:1rem;font-weight:800;color:var(--text);display:flex;align-items:center;gap:.55rem;letter-spacing:-.02em}
.card-header h2 i{color:var(--purple-mid);font-size:.88rem}
.card-badge{background:#f59e0b;color:#1a1a00;font-size:.68rem;font-weight:800;padding:.18rem .55rem;border-radius:6px;letter-spacing:.03em}
.card-link{display:inline-flex;align-items:center;gap:.35rem;font-size:.78rem;font-weight:700;color:var(--purple-mid);text-decoration:none}
.card-link:hover{opacity:.8}
.section-stack{display:flex;flex-direction:column;gap:1.25rem}

/* Rules */
.centered-header{padding:.95rem 1rem;margin-bottom:1rem;border-radius:16px;background:linear-gradient(135deg,#f0fdfa,#ffffff);border:1px solid #ccfbf1;text-align:center}
.centered-header p:first-child{font-family:var(--ff);font-size:1rem;font-weight:800;color:var(--text)}
.centered-header p:last-child{font-size:.8rem;color:var(--muted);margin-top:.15rem}
.rules-list{display:flex;flex-direction:column;gap:.65rem;max-height:540px;overflow:auto;padding-right:.2rem}
.rule-item{display:flex;gap:.75rem;padding:.9rem 1rem;border-radius:16px;background:linear-gradient(135deg,#fbfefd,#f0fdfa);border:1px solid #d6f5ef}
.rule-number{width:30px;height:30px;border-radius:10px;background:linear-gradient(135deg,var(--purple-mid),var(--purple-light));color:#fff;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:800;flex-shrink:0;box-shadow:0 10px 20px rgba(15,118,110,.18)}
.rule-text{font-size:.82rem;line-height:1.62;color:#4b5563}

/* Reservation / resources helpers */
.soft-banner{display:flex;align-items:center;gap:1rem;padding:1.05rem 1.2rem;background:linear-gradient(135deg,#ffffff,#f0fdfa);border:1px solid #d6f5ef;border-radius:18px;box-shadow:0 10px 24px rgba(15,118,110,.05)}
.soft-banner-icon{width:48px;height:48px;border-radius:15px;background:linear-gradient(135deg,var(--purple-mid),var(--purple-light));display:flex;align-items:center;justify-content:center;font-size:1.18rem;color:#fff;flex-shrink:0;box-shadow:0 12px 24px rgba(15,118,110,.20)}
.filter-select{padding:.56rem .9rem;border:1.5px solid #cfe9e5;border-radius:12px;background:#fff;font-size:.82rem;font-weight:600;color:var(--text);outline:none;min-width:180px;box-shadow:0 6px 16px rgba(15,118,110,.04)}
.filter-select:focus{border-color:var(--purple-mid);box-shadow:0 0 0 4px rgba(15,118,110,.10)}
...
/* Content Topbar */
.dash-content-topbar{display:flex;align-items:center;justify-content:flex-end;gap:1.25rem;margin:-1.8rem -2rem 2.2rem;padding:.8rem 2.5rem .7rem 2rem;background:linear-gradient(180deg,rgba(248,246,255,.92),rgba(248,246,255,.72));backdrop-filter:blur(14px);border-bottom:1px solid rgba(235,229,249,.85);position:sticky;top:0;z-index:200;box-shadow:0 10px 28px rgba(58,34,112,.04)}
...
/* Responsive */
@media(max-width:1100px){.stat-cards{grid-template-columns:repeat(2,1fr)};.reservation-grid{grid-template-columns:repeat(2,1fr)};.profile-layout{grid-template-columns:1fr}.hero-mini-stats{grid-template-columns:repeat(3,1fr)}}
@media(max-width:900px){.home-grid{grid-template-columns:1fr}.welcome-badge{align-items:flex-start}.hero-mini-stats{width:100%}}
@media(max-width:768px){
  ...
  .welcome-banner{padding:1.4rem}
  .hero-mini-stats{grid-template-columns:1fr}
}
@media(max-width:500px){.stat-cards{grid-template-columns:1fr 1fr};.history-summary{flex-direction:column};.welcome-title{font-size:1.35rem}.page-title{font-size:1.45rem}}
  </style>
</head>
...
<aside class="sidebar" id="sidebar">
  <div class="sb-brand">
    <div class="sb-brand-text">
      <span class="sb-title">Student Portal</span>
    </div>
  </div>
  <nav class="sb-nav">
    <ul>
  ...
  <div class="dct-right" style="margin-left:auto">
    ...
    <div class="dct-profile-chip" id="dctProfileChip">
      ...
      <div class="dct-profile-dropdown" id="dctProfileDropdown">
        ...
        <a href="dashboard.php?logout=1" class="dct-dd-item danger"><i class="fa-solid fa-right-from-bracket"></i> Logout</a>
      </div>
    </div>
  </div>
</div>
...
<div class="notif-panel" id="notifPanel">
  ...
  <div class="dash-page" id="page-home">
    <div class="welcome-banner">
      <div class="welcome-text">
        ...
        <p class="welcome-sub">Here’s a live snapshot of your sit-in activity, reservations, and lab updates so you can move through the day faster.</p>
        <div class="welcome-actions">
          <button class="btn-primary-sm" type="button" onclick="openReserveModal()"><i class="fa-solid fa-calendar-plus"></i> Reserve a Lab</button>
          <button class="btn-outline-sm" type="button" onclick="window._dashSwitchPage && window._dashSwitchPage('resources')"><i class="fa-solid fa-file-arrow-down"></i> View Resources</button>
        </div>
        <div class="hero-mini-stats">
          <div class="mini-stat">
            <span class="mini-stat-label"><i class="fa-solid fa-clock"></i> Remaining</span>
            <span class="mini-stat-value"><?= $sessions_left ?></span>
          </div>
          <div class="mini-stat">
            <span class="mini-stat-label"><i class="fa-solid fa-star"></i> Score</span>
            <span class="mini-stat-value"><?= $student_score ?></span>
          </div>
          <div class="mini-stat">
            <span class="mini-stat-label"><i class="fa-solid fa-hourglass-half"></i> Hours</span>
            <span class="mini-stat-value"><?= $student_hours ?></span>
          </div>
        </div>
      </div>
      <div class="welcome-badge">
        ...
        <div class="welcome-status"><i class="fa-solid fa-circle"></i> Account Active</div>
      </div>
    </div>
    ...
    <div class="dash-card">
      <div class="card-header">
        ...
        <a href="#history" class="card-link" onclick="event.preventDefault(); window._dashSwitchPage && window._dashSwitchPage('history')">See activity <i class="fa-solid fa-arrow-right"></i></a>
      </div>
      ...
      <div class="dash-card">
        <div class="card-header">
          <h2><i class="fa-solid fa-wave-square"></i> Quick Snapshot</h2>
          <span class="card-badge">Live</span>
        </div>
        <div class="soft-banner">
          <div class="soft-banner-icon"><i class="fa-solid fa-chart-line"></i></div>
          <div style="flex:1;min-width:0">
            <div style="font-family:var(--ff);font-size:1rem;font-weight:800;color:var(--text)">Progress This Term</div>
            <div style="font-size:.82rem;color:var(--muted);margin-top:.22rem;line-height:1.55">You’ve completed <strong style="color:var(--text)"><?= $sum_sessions ?></strong> sit-in session<?= $sum_sessions !== 1 ? 's' : '' ?> with an average duration of <strong style="color:var(--text)"><?= htmlspecialchars($avg_duration) ?></strong>.</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <!-- Rules -->
      <div class="dash-card">
        <div class="card-header"><h2><i class="fa-solid fa-clipboard-list"></i> Rules and Regulations</h2><span class="card-badge">Read before sit-in</span></div>
  ...
  <div class="dash-page" id="page-reservation">
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Book a computer lab slot in advance, review software availability, and choose a preferred workstation before you arrive.</p>
      </div>
  ...
  <div class="dash-page" id="page-resources">
    <div class="page-header">
      ...
      <div class="page-actions">
        <select id="resourcesCategoryFilter" class="filter-select">
          <option value="all">All Categories</option>
          <option value="general">General</option>
          <option value="announcement">Announcements</option>
          <option value="schedule">Schedules</option>
          <option value="report">Reports</option>
          <option value="resource">Learning Resources</option>
  ...
  <div class="modal-card" style="max-width:700px">
    ...
    <div id="reserveStep1">
      <form id="reserveForm" class="auth-form" style="gap:.8rem;padding:0 1.8rem 1.8rem">
        ...
        <input type="hidden" id="res_software" name="res_software" value="">
...
<script>
const labSoftwareMap = <?= json_encode($lab_software_map, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>;
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openReserveModal() {
    ...
    renderLabSoftware(document.getElementById('res_lab')?.value || '');
  ...
  function renderLabSoftware(labName) {
    const software = labSoftwareMap[labName] || [];
    const previewWrap = document.getElementById('resLabSoftwarePreview');
    const previewChips = document.getElementById('resLabSoftwareChips');
    const noSoftware = document.getElementById('resLabNoSoftware');
    const fullSection = document.getElementById('softwareSection');
    const fullList = document.getElementById('softwareList');
    const hiddenInput = document.getElementById('res_software');

    if (hiddenInput) {
      hiddenInput.value = software.join(', ');
    }

    if (!labName) {
      if (previewWrap) previewWrap.style.display = 'none';
      if (noSoftware) noSoftware.style.display = 'none';
    ...
    if (!software.length) {
      if (previewWrap) previewWrap.style.display = 'none';
      if (noSoftware) noSoftware.style.display = 'block';
      if (fullSection) fullSection.style.display = 'none';
      return;
    }

    const chips = software.map(item =>
      `<span class="software-chip"><i class="fa-solid fa-check"></i>${item}</span>`
    ).join('');

    if (previewChips) previewChips.innerHTML = chips;
    if (fullList) fullList.innerHTML = chips;
    if (previewWrap) previewWrap.style.display = 'block';
    if (noSoftware) noSoftware.style.display = 'none';
    if (fullSection) fullSection.style.display = 'block';
  }

  document.getElementById('res_lab')?.addEventListener('change', (event) => {
    renderLabSoftware(event.target.value);
  });
  renderLabSoftware(document.getElementById('res_lab')?.value || '');
  ...
  document.getElementById('submitReservationBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        document.getElementById('res_software').value = '';
        renderLabSoftware('');
        _smSelectedPc = null;
        document.getElementById('res_pc_hidden').value = '';
...
(function() {
  ...
  window.openPdfPreview = function(viewUrl, name, downloadUrl) {
    ...
    const zoomedViewUrl = viewUrl.includes('#') ? viewUrl : `${viewUrl}#zoom=page-width`;
    document.getElementById('pdfPreviewTitle').textContent = name;
    document.getElementById('pdfPreviewFrame').src = zoomedViewUrl;
